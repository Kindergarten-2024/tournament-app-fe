...
const Clicker = () => {
  ...
  const ballImages = [ballImage1, ballImage2, ballImage3, ballImage4, ballImage5, ballImage6, ballImage7];
  ...
  const handleButtonClick = () => {
    ...
    setTimeout(() => {
      setBalls(currentBalls => currentBalls.filter(ball => ball.id !== newBall.id));
    }, 3000);
  };

  return (
    <div className='clicker-icon'>
      <img onClick={handleButtonClick} src={logo2} alt="logo2" width="95" height="200" />
      {balls.map(ball => (
        <div key={ball.id} className={ball.className} style={{
          left: ball.left + 'px',
          top: ball.top + 'px',
